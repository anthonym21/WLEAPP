...
def get_betterDiscord(files_found, report_folder, seeker, wrap_text):
    ...
    for file_found in files_found:
        ...
        if os.path.basename(file_found) != 'MessageLoggerV2Data.config.json': # skip -journal and other files
            continue
                ...
                for keylin, valuein in value.items():
                    ...
                    if referencing:
                        ...
                        while control:
                            ...
                            aggregator = f'{aggregator}<td>{timestamp_in}</td>'
                            aggregator = f'{aggregator}<td>{username_in}</td>'
                            aggregator = f'{aggregator}<td>{content_in}</td>'
                            aggregator = f'{aggregator}</tr>'
                            if referencing:
                                control = 1
                            else:
                                control = 0
                                aggregator = f'{aggregator}</table>'
    ...
    if num_entries > 0:
        ...
        tsvname = 'Better Discord'
        tsv(report_folder, data_headers, data_list, tsvname)

        tlactivity = 'Better Discord'
