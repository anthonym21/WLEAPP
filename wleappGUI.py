...
def ValidateInput(values, window):
    ...
    one_element_is_selected = any(
        window.FindElement(x).Get() for x in range(1000, indx)
    )
...
def CheckList(mtxt, lkey, mdstring, disable=False):
    dstate = mdstring not in ['test1', 'test2']
...
def pickModules():
    ...
    sorted_tosearch = dict(
        sorted(tosearch.items(), key=lambda item: item[1][0].upper())
    )

    indx = 1000     # arbitrary number to not interfere with other controls
    for key, val in sorted_tosearch.items():
        disabled = key == 'usagestatsVersion'
        mlist.append(CheckList(f'{val[0]} [{key}]', indx, key, disabled))
        indx = indx + 1
...
pickModules()
GuiWindow.progress_bar_total = len(wleapp.tosearch)


layout = [  [sg.Text('Windows Logs, Events, And Properties Parser', font=("Helvetica", 22))],
            [sg.Text('https://github.com/abrignoni/WLEAPP', font=("Helvetica", 14))],
            [sg.Frame(layout=[
                    [sg.Input(size=(97,1)), 
...
# Create the Window
window = sg.Window(f'WLEAPP version {wleapp_version}', layout)
...
while True:
    ...
    if event == "DESELECT ALL":  
        ...
        for x in range(1000,indx):
            ...
            window[x].Update(window[x].metadata == 'usagestatsVersion')
    if event == 'Process':
        ...
        if is_valid:
            ...
            wrap_text = True
            if crunch_successful := wleapp.crunch_artifacts(
                search_list,
                extracttype,
                input_path,
                out_params,
                len(wleapp.tosearch) / s_items,
                wrap_text,
            ):
